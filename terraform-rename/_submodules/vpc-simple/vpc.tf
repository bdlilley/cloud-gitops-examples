...
resource "aws_vpc" "vpc" {
  ...
  tags                 = { "Name" : "${var.resourcePrefix}-${var.region}" }
}
...
resource "aws_subnet" "private" {
  ...
  availability_zone_id = each.value.zoneId

  tags = {
    Name                              = "${var.resourcePrefix}-private-${each.value.zoneId}"
...
resource "aws_subnet" "public" {
  ...
  tags = {
    Name = "${var.resourcePrefix}-public-${each.value.zoneId}"
...
resource "aws_security_group" "interface" {
  name        = "${var.resourcePrefix}-interface"
